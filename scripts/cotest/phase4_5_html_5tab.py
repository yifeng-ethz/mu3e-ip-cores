  ...
  function renderRdma(rid, data){
    ...
    if (frames.length) {
      ...
      frames.forEach(function(f){
        ...
        // each 32-bit word is rendered MSB-first so a human reader sees the
        // word as it would appear written out (e.g. 0xBC at the END of the
        // preamble word makes the K28.5 marker easy to spot). The byte
        // offset annotation is still the disk offset.
        var hexLines = '';
        for (var off = 0; off < hex.length; off += 32) {
          ...
          for (var b = 0; b < 32; b += 8) {
            ...
            var wb0 = hex.slice(idx,     idx + 2); // LSB on disk
