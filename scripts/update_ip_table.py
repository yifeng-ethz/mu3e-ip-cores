...
def list_submodules() -> list[tuple[str, str]]:
    """Return list of (path, url) tuples from .gitmodules in repo order."""
    out = run(
        [
            "git",
            "config",
...
def build_row(path: str, url: str, readme_text: str) -> IpRow:
...
def render_table(rows: list[IpRow]) -> str:
    lines = [
        ...
        " Do not edit by hand. -->",
        "",
        "| IP | Description | Version | Last Updated | Signoff | HEAD |",
        "|---|---|:---:|:---:|:---:|:---:|",
    ]
    for r in rows:
        ...
        head_cell = (
            f"`{r.head_sha}`"
            + (f" ({r.branch})" if r.branch and r.branch != "master" else "")
        )
        lines.append(
            f"| {name_cell} | {desc} | `{r.version}` | "
            f"{r.last_updated} | {r.signoff} | {head_cell} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str]) -> int:
    ...
    rows: list[IpRow] = []
    for path, url in submods:
        rows.append(build_row(path, url, readme_text))
