...
proc elaborate {} {
        ...
        set_display_item_property source_overview_html TEXT [format {<html><b>Runtime lane source select</b><br/>This IP forwards either the real LVDS/deassembly stream or the local emulator stream into one MuTRiG datapath lane. Both inputs remain connected so firmware can run all lanes real, all lanes emulated, or any mixed lane mask through the CSR window.<br/><br/><b>Reset default</b><br/>This instance resets to the <b>%s</b> source. Software may switch it later by writing CONTROL.select_emulator.</html>} $default_label]
    }
}
...
add_html_text "Overview" source_overview_html {<html><b>Runtime lane source select</b><br/>This IP forwards either the real LVDS/deassembly stream or the local emulator stream into one MuTRiG datapath lane.</html>}
add_html_text "Reset Policy" reset_policy_html {<html><b>Safe switching</b><br/>Software should switch sources while the run is stopped or the lane is otherwise quiescent. The selector is synchronous, but it does not frame-align or drain either 8b/1k stream before changing source.</html>}
...
add_html_text "Streams" streams_html {<html><b>real_in</b><br/>9-bit decoded MuTRiG stream from the LVDS controller.<br/><br/><b>emu_in</b><br/>9-bit 8b/1k stream from emulator_mutrig.<br/><br/><b>selected_out</b><br/>The selected stream. Error and channel sidebands are forwarded without modification.</html>}
add_html_text "Control" control_html {<html><b>csr</b><br/>16-word Avalon-MM slave aperture. Each lane has a separate instance so mixed real/emulator operation is a lane mask in software.</html>}
...
<tr><td>0x02</td><td>CONTROL</td><td>RW</td><td>Bit 0 select_emulator: 0=real, 1=emulator. Bit 1 clears counters when written as one.</td></tr>
<tr><td>0x03</td><td>STATUS</td><td>RO</td><td>Live select plus current valid/error/channel sidebands for real, emulator, and selected output.</td></tr>
...
<tr><td>0x07</td><td>SWITCH_COUNT</td><td>RO</td><td>Saturating count of source-select changes through CONTROL.</td></tr>
