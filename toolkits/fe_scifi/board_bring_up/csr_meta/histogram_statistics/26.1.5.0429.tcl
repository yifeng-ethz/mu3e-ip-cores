...
namespace eval ::fe_scifi::board_bring_up::csr_meta::histogram_statistics::v26_1_5_0429 {}

proc ::fe_scifi::board_bring_up::csr_meta::histogram_statistics::v26_1_5_0429::get_metadata {} {
    set registers [list \
        [::fe_scifi::board_bring_up::csr_meta::register \
            "uid" \
            "Software-visible IP identifier. Default payload is ASCII HIST (0x48495354)." \
            "0x0" \
            [list \
        ...
        [::fe_scifi::board_bring_up::csr_meta::register \
            ...
            "0x4" \
            [list \
                [::fe_scifi::board_bring_up::csr_meta::field "version" "Packed version word. Write 0 to select this page before reading." {[31:0]} "read-only"]] \
        ...
        [::fe_scifi::board_bring_up::csr_meta::register \
            ...
            "0x8" \
            [list \
                [::fe_scifi::board_bring_up::csr_meta::field "commit" "Write 1 to commit the pending histogram settings." {[0:0]} "read-write"] \
                [::fe_scifi::board_bring_up::csr_meta::field "cfg_apply_pending" "1 while a committed configuration is waiting for the ingress path to drain." {[1:1]} "read-only"] \
                [::fe_scifi::board_bring_up::csr_meta::field "mode" "Histogram operating mode selector. Negative two's-complement values select debug inputs. In debug modes the filter compares a synthetic word: \[15:0\] sample, \[23:16\] zero-based debug source, \[31:24\] absolute debug mode." {[7:4]} "read-write"] \
                [::fe_scifi::board_bring_up::csr_meta::field "representation" "Update-key representation: 0 signed, 1 unsigned." {[8:8]} "read-write"] \
                [::fe_scifi::board_bring_up::csr_meta::field "filter" "Filter mode encoded as bits {reject, enable}: 0 disables filtering, 1 keeps matches, 3 rejects matches." {[13:12]} "read-write"] \
                ...
                [::fe_scifi::board_bring_up::csr_meta::field "error_info" "Validation error code for the last rejected commit." {[31:28]} "read-only"]]] \
        [::fe_scifi::board_bring_up::csr_meta::register \
            "left_bound" \
        ...
        [::fe_scifi::board_bring_up::csr_meta::register \
            "right_bound" \
            "Upper edge of the histogram range. Recomputed at commit time when bin_width is non-zero." \
            "0x10" \
            [list \
                [::fe_scifi::board_bring_up::csr_meta::field "right_bound" "Upper bound used for the binning range." {[31:0]} "read-write"]]] \
        [::fe_scifi::board_bring_up::csr_meta::register \
            "bin_width" \
            "Width of one histogram bin in update-key units." \
            "0x14" \
            [list \
                [::fe_scifi::board_bring_up::csr_meta::field "bin_width" "Bin width in update-key units." {[15:0]} "read-write"]]] \
        [::fe_scifi::board_bring_up::csr_meta::register \
            "keys_location" \
            "Bit positions of the update and filter keys within the snooped data word. In debug modes the filter slice selects the synthetic debug word." \
            "0x18" \
            [list \
        ...
        [::fe_scifi::board_bring_up::csr_meta::register \
            "underflow_counter" \
            "Events where the extracted key fell below the configured histogram range." \
            "0x20" \
            [list \
                [::fe_scifi::board_bring_up::csr_meta::field "underflow_cnt" "Number of update-key underflow events since clear or interval rollover." {[31:0]} "read-only"]]] \
        [::fe_scifi::board_bring_up::csr_meta::register \
            "overflow_counter" \
            "Events where the extracted key exceeded the configured histogram range." \
            "0x24" \
            [list \
                [::fe_scifi::board_bring_up::csr_meta::field "overflow_cnt" "Number of update-key overflow events since clear or interval rollover." {[31:0]} "read-only"]]] \
        [::fe_scifi::board_bring_up::csr_meta::register \
            "interval_cfg" \
        ...
        [::fe_scifi::board_bring_up::csr_meta::register \
            ...
            "0x2c" \
            [list \
                [::fe_scifi::board_bring_up::csr_meta::field "active_bank" "Currently active write bank." {[0:0]} "read-only"] \
