...
Phase-5 frame/MTS/histogram runtime captures. The first MTS timestamp-error
trigger attempt timed out because the real-lane retry no longer produced
...
strict retry. However, the later histogram-triggered real run records one MTS
discard while still producing accepted histogram hits. Treat the real-MuTRiG
Phase-5 BASIC rows as still blocked for closure until:
...
2. The MTS error trigger is kept available for a soak run to catch the next
   intermittent timestamp-error event.
