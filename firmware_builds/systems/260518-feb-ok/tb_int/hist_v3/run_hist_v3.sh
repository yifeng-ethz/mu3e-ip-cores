...
if [[ -n "${MU3E_IP_CORES_ROOT:-}" ]]; then
    ...
    BUILD_DIR="${BUILD_DIR:-${REPO_DIR}/firmware_builds/systems/260518-feb-ok}"
else
    BUILD_DIR="$(realpath -m -- "${SCRIPT_DIR}/../..")"
    REPO_DIR="$(realpath -m -- "${BUILD_DIR}/../../..")"
fi
TB_DIR="${BUILD_DIR}/tb_int/hist_v3"
REPORT_DIR="${BUILD_DIR}/tb_int/REPORT"
SIM_RTL_DIR="${BUILD_DIR}/generated/simulation/scifi_datapath_system_v3/simulation/submodules"
