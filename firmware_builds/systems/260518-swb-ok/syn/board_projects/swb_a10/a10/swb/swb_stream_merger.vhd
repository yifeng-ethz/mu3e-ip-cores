...
port (
    -- input streams
    i_rdata     : in    work.mu3e.link64_array_t(N-1 downto 0);
...
architecture arch of swb_stream_merger is
    ...
    -- stream merger
    signal stream_merger_rdata : std_logic_vector(N*work.mu3e.LINK_64B_LENGTH-1 downto 0);
    signal stream_merger_wdata : std_logic_vector(work.mu3e.LINK_64B_LENGTH-1 downto 0);
    ...
    signal wdata_debug : work.mu3e.link64_t;
...
begin
    ...
    process(i_rdata)
        variable v_rdata : std_logic_vector(work.mu3e.LINK_64B_LENGTH-1 downto 0);
    begin
        for i in i_rdata'range loop
            v_rdata := work.mu3e.to_slv(i_rdata(i));
            stream_merger_rdata((i+1)*work.mu3e.LINK_64B_LENGTH-1 downto i*work.mu3e.LINK_64B_LENGTH) <= v_rdata;
        end loop;
    end process;
    ...
    e_stream_fifo_farm : entity work.link64_scfifo
    generic map (
