...
| [BUG-008-H](#bug-008-h-phase4_5-html-reports-showed-raw-hit-count-delta-without-run-window-normalization) | H | non-datapath-refactor | `common (every row in cross-compare HTML)` | fixed | Phase 4.5 sweep cross-compare report, 2026-05-12 | this commit | Raw hit-count delta column made sim (10 ms) vs board (4000 ms) look like 400x divergence; delta values of +15000% for PASSed rows obscured real behavioral failures at +inf% (sanity-neg inversion). |
...
### BUG-008-H: phase4_5 HTML reports showed raw hit-count delta without run-window normalization

- First seen:
  - Phase 4.5 cross-compare HTML generated by commit 967845b5, 2026-05-12.
- Symptom:
  - The delta% column for PASSed rows showed +15000% to +170000%, which looked like
    catastrophic disagreement. The actual root cause was a 400x wallclock mismatch:
    sim ran 10 ms (1,250,000 cycles @ 125 MHz) while board ran 4000 ms (4 s).
    Rows p45_025 and p45_031 showing +inf% (sanity-neg with sim=0 and board>0)
    were visually indistinguishable from false alarms in the raw-count table.
- Root cause:
  - PHASE4_5_SWEEP_REPORT.html was rendered with raw absolute hit counts for both
    sim and board. No per-millisecond normalization was applied, so the comparison
    column showed (board_total - sim_total) / sim_total * 100 which is dominated
    by the 400x run-window ratio rather than any functional difference.
- Fix:
  - `scripts/cotest/phase4_5_html_normalize.py` added as standalone helper.
  - Both HTML reports updated with sim_hits/ms, board_hits/ms, and delta_pct_rate
    columns. All 32 per-row counter comparison tables expanded to 9 columns.
  - New "Run Window Normalization" section inserted in both HTML files.
- Evidence:
  - p45_000 (baseline): raw delta was +15370.9%; rate delta is -61.3% (FIFO saturation
    at 4 s board window vs 10 ms sim, not a behavioral failure).
  - p45_025 (sanity-neg inversion): rate delta is +inf% correctly marking real failure.
  - p45_019 (rate-knob 0x4000): sim 164991.20 hits/ms vs board 167507.38 hits/ms,
    delta_pct_rate +1.5% -- near-perfect agreement despite both sides being FAIL
    on the dropped-hits predicate.
