...
# STP probe fabric is debug-only and can be armed without making probe cells
# functional timing endpoints. This exception is limited to SignalTap cells;
# system nodes that feed the probes remain timed normally.
...
# multicycle path: LVDS CSR bridge command feedback inside data_path_subsystem
# The mm_pipeline_avmm_lvds bridge feeds sparse slow-control CSR transactions
# from the system master into the LVDS/data-path CSR fabric. The failing setup
# cone is Merlin command feedback from CSR burst adapters and the bridge
# translator's begin-transfer lookahead back into the same bridge address/read
# registers. These paths limit only single-cycle back-to-back CSR command
# issue; board bring-up and software access use isolated slow-control
# transactions, not full-rate streaming through this CSR bridge.
set lvds_csr_cmd_feedback [get_keepers -nowarn {*data_path_subsystem*mm_interconnect_1*burst_adapter*in_ready_hold}]
set lvds_csr_begintransfer [get_keepers -nowarn {*data_path_subsystem*mm_interconnect_1*mm_pipeline_avmm_lvds_m0_translator*end_begin*transfer*}]
set lvds_csr_feedback_src [add_to_collection $lvds_csr_cmd_feedback $lvds_csr_begintransfer]
set lvds_csr_addr_dst [get_keepers -nowarn {*data_path_subsystem*mm_interconnect_1*mm_pipeline_avmm_lvds_m0_translator*address_register*}]
set lvds_csr_bridge_dst [get_keepers -nowarn {*data_path_subsystem*mm_pipeline_avmm_lvds*wr_reg_read}]
set lvds_csr_feedback_dst [add_to_collection $lvds_csr_addr_dst $lvds_csr_bridge_dst]
if { [get_collection_size $lvds_csr_feedback_src] > 0 && [get_collection_size $lvds_csr_feedback_dst] > 0 } {
    set_multicycle_path -setup -end 2 -from $lvds_csr_feedback_src -to $lvds_csr_feedback_dst
    set_multicycle_path -hold  -end 1 -from $lvds_csr_feedback_src -to $lvds_csr_feedback_dst
}
