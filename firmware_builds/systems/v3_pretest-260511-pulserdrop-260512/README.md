...
Verdict: **FAIL at integration 1.0x nominal clocks**. The active board STA
inputs do not contain a standalone-IP 1.1x clock scale; `top.qip` loads
`src/top.sdc` and `src/fe/top_FEB_v2.sdc`, whose base clocks are nominal. A
no-refit `quartus_sta top -c top` rerun still reports Slow85 setup WNS
`-1.245 ns`, Slow0 setup WNS `-1.076 ns`, Slow85 recovery WNS `-1.344 ns`, and
Slow0 recovery WNS `-1.243 ns`.

No SOF promotion from this build. Detailed trace:
[`doc/STA_FAIL_TRACE.md`](doc/STA_FAIL_TRACE.md).
