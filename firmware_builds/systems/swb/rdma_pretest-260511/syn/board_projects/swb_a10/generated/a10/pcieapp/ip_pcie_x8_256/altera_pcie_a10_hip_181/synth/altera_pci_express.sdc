...
if {[get_collection_size $pll_pcie_clock] == 1} {												;# single instance
		...
		foreach clk [split $clk_prefix " "] { 																	;# store each unique parent to all_clk_parent_list
...
if {[get_collection_size $pll_pcie_clock] == 1} {												;# single instance
		set byte_ser_clk_pin0 [lindex [split [query_collection $byte_ser_clk_pins] " "] 0]		;# one type of pin prefix	
		...
		foreach clk [split $byte_ser_clk_pin0 " "] {
