-- lcpll_g3xn.vhd

-- Generated using ACDS version 18.1 625

library IEEE;
library ip_pcie_x4_256_altera_xcvr_atx_pll_a10_181;
use IEEE.std_logic_1164.all;
use IEEE.numeric_std.all;
use ip_pcie_x4_256_altera_xcvr_atx_pll_a10_181.lcpll_g3xn_pkg.all;

entity lcpll_g3xn is
	generic (
		enable_pll_reconfig                                              : integer := 1;
		rcfg_jtag_enable                                                 : integer := 0;
		rcfg_separate_avmm_busy                                          : integer := 0;
		dbg_embedded_debug_enable                                        : integer := 0;
		dbg_capability_reg_enable                                        : integer := 0;
		dbg_user_identifier                                              : integer := 0;
		dbg_stat_soft_logic_enable                                       : integer := 0;
		dbg_ctrl_soft_logic_enable                                       : integer := 0;
		rcfg_emb_strm_enable                                             : integer := 0;
		rcfg_profile_cnt                                                 : integer := 2;
		hssi_pma_lc_refclk_select_mux_powerdown_mode                     : string  := "powerup";
		hssi_pma_lc_refclk_select_mux_refclk_select                      : string  := "ref_iqclk0";
		hssi_pma_lc_refclk_select_mux_silicon_rev                        : string  := "20nm5";
		hssi_pma_lc_refclk_select_mux_inclk0_logical_to_physical_mapping : string  := "ref_iqclk0";
		hssi_pma_lc_refclk_select_mux_inclk1_logical_to_physical_mapping : string  := "power_down";
		hssi_pma_lc_refclk_select_mux_inclk2_logical_to_physical_mapping : string  := "power_down";
		hssi_pma_lc_refclk_select_mux_inclk3_logical_to_physical_mapping : string  := "power_down";
		hssi_pma_lc_refclk_select_mux_inclk4_logical_to_physical_mapping : string  := "power_down";
		hssi_refclk_divider_silicon_rev                                  : string  := "20nm5";
		atx_pll_silicon_rev                                              : string  := "20nm5";
		atx_pll_is_cascaded_pll                                          : string  := "false";
		atx_pll_cgb_div                                                  : integer := 1;
		atx_pll_pma_width                                                : integer := 32;
		atx_pll_cp_compensation_enable                                   : string  := "true";
		atx_pll_cp_current_setting                                       : string  := "cp_current_setting26";
		atx_pll_cp_testmode                                              : string  := "cp_normal";
		atx_pll_cp_lf_3rd_pole_freq                                      : string  := "lf_3rd_pole_setting0";
		atx_pll_lf_cbig_size                                             : string  := "lf_cbig_setting4";
		atx_pll_cp_lf_order                                              : string  := "lf_3rd_order";
		atx_pll_lf_resistance                                            : string  := "lf_setting1";
		atx_pll_lf_ripplecap                                             : string  := "lf_ripple_cap_0";
		atx_pll_tank_sel                                                 : string  := "lctank0";
		atx_pll_tank_band                                                : string  := "lc_band3";
		atx_pll_tank_voltage_coarse                                      : string  := "vreg_setting_coarse0";
		atx_pll_tank_voltage_fine                                        : string  := "vreg_setting5";
		atx_pll_output_regulator_supply                                  : string  := "vreg1v_setting0";
		atx_pll_overrange_voltage                                        : string  := "over_setting0";
		atx_pll_underrange_voltage                                       : string  := "under_setting4";
		atx_pll_fb_select                                                : string  := "direct_fb";
		atx_pll_d2a_voltage                                              : string  := "d2a_setting_4";
		atx_pll_dsm_mode                                                 : string  := "dsm_mode_integer";
		atx_pll_dsm_out_sel                                              : string  := "pll_dsm_disable";
		atx_pll_dsm_ecn_bypass                                           : string  := "false";
		atx_pll_dsm_ecn_test_en                                          : string  := "false";
		atx_pll_dsm_fractional_division                                  : string  := "1";
		atx_pll_dsm_fractional_value_ready                               : string  := "pll_k_ready";
		atx_pll_iqclk_mux_sel                                            : string  := "iqtxrxclk0";
		atx_pll_vco_bypass_enable                                        : string  := "false";
		atx_pll_l_counter                                                : integer := 2;
		atx_pll_l_counter_enable                                         : string  := "true";
		atx_pll_cascadeclk_test                                          : string  := "cascadetest_off";
		atx_pll_hclk_divide                                              : integer := 50;
		atx_pll_enable_hclk                                              : string  := "hclk_disabled";
		atx_pll_m_counter                                                : integer := 40;
		atx_pll_ref_clk_div                                              : integer := 1;
		atx_pll_bw_sel                                                   : string  := "high";
		atx_pll_datarate                                                 : string  := "8000000000 bps";
		atx_pll_device_variant                                           : string  := "device1";
		atx_pll_initial_settings                                         : string  := "true";
		atx_pll_lc_mode                                                  : string  := "lccmu_normal";
		atx_pll_output_clock_frequency                                   : string  := "4000000000 Hz";
		atx_pll_powerdown_mode                                           : string  := "powerup";
		atx_pll_prot_mode                                                : string  := "pcie_gen3_tx";
		atx_pll_reference_clock_frequency                                : string  := "100000000 Hz";
		atx_pll_sup_mode                                                 : string  := "user_mode";
		atx_pll_regulator_bypass                                         : string  := "reg_enable";
		atx_pll_vco_freq                                                 : string  := "8000000000 Hz";
		atx_pll_is_otn                                                   : string  := "false";
		atx_pll_is_sdi                                                   : string  := "false";
		atx_pll_primary_use                                              : string  := "hssi_x1";
		atx_pll_fpll_refclk_selection                                    : string  := "select_vco_output";
		atx_pll_lc_to_fpll_l_counter_scratch                             : integer := 1;
		atx_pll_lc_to_fpll_l_counter                                     : string  := "lcounter_setting0";
		atx_pll_pfd_delay_compensation                                   : string  := "normal_delay";
		atx_pll_xcpvco_xchgpmplf_cp_current_boost                        : string  := "normal_setting";
		atx_pll_pfd_pulse_width                                          : string  := "pulse_width_setting0";
		hip_cal_en                                                       : string  := "enable";
		calibration_en                                                   : string  := "enable";
		enable_analog_resets                                             : integer := 0;
		atx_pll_bonding_mode                                             : string  := "cpri_bonding";
		enable_mcgb                                                      : integer := 1;
		enable_mcgb_debug_ports_parameters                               : integer := 0;
		hssi_pma_cgb_master_prot_mode                                    : string  := "pcie_gen3_tx";
		hssi_pma_cgb_master_silicon_rev                                  : string  := "20nm5";
		hssi_pma_cgb_master_x1_div_m_sel                                 : string  := "divbypass";
		hssi_pma_cgb_master_cgb_enable_iqtxrxclk                         : string  := "disable_iqtxrxclk";
		hssi_pma_cgb_master_ser_mode                                     : string  := "thirty_two_bit";
		hssi_pma_cgb_master_datarate                                     : string  := "8000000000 bps";
		hssi_pma_cgb_master_cgb_power_down                               : string  := "normal_cgb";
		hssi_pma_cgb_master_observe_cgb_clocks                           : string  := "observe_nothing";
		hssi_pma_cgb_master_op_mode                                      : string  := "enabled";
		hssi_pma_cgb_master_tx_ucontrol_reset_pcie                       : string  := "pcscorehip_controls_mcgb";
		hssi_pma_cgb_master_vccdreg_output                               : string  := "vccdreg_nominal";
		hssi_pma_cgb_master_input_select                                 : string  := "fpll_top";
		hssi_pma_cgb_master_input_select_gen3                            : string  := "lcpll_top"
	);
	port (
		pll_powerdown         : in  std_logic                     := '0';             --     pll_powerdown.pll_powerdown
		pll_refclk0           : in  std_logic                     := '0';             --       pll_refclk0.clk
		pll_locked            : out std_logic;                                        --        pll_locked.pll_locked
		pll_pcie_clk          : out std_logic;                                        --      pll_pcie_clk.pll_pcie_clk
		reconfig_clk0         : in  std_logic                     := '0';             --     reconfig_clk0.clk
		reconfig_reset0       : in  std_logic                     := '0';             --   reconfig_reset0.reset
		reconfig_write0       : in  std_logic                     := '0';             --    reconfig_avmm0.write
		reconfig_read0        : in  std_logic                     := '0';             --                  .read
		reconfig_address0     : in  std_logic_vector(9 downto 0)  := (others => '0'); --                  .address
		reconfig_writedata0   : in  std_logic_vector(31 downto 0) := (others => '0'); --                  .writedata
		reconfig_readdata0    : out std_logic_vector(31 downto 0);                    --                  .readdata
		reconfig_waitrequest0 : out std_logic;                                        --                  .waitrequest
		pll_cal_busy          : out std_logic;                                        --      pll_cal_busy.pll_cal_busy
		hip_cal_done          : out std_logic;                                        --      hip_cal_done.hip_cal_done
		mcgb_rst              : in  std_logic                     := '0';             --          mcgb_rst.mcgb_rst
		mcgb_aux_clk0         : in  std_logic                     := '0';             --     mcgb_aux_clk0.tx_serial_clk
		tx_bonding_clocks     : out std_logic_vector(5 downto 0);                     -- tx_bonding_clocks.clk
		pcie_sw               : in  std_logic_vector(1 downto 0)  := (others => '0'); --           pcie_sw.pcie_sw
		pcie_sw_done          : out std_logic_vector(1 downto 0);                     --      pcie_sw_done.pcie_sw_done
		mcgb_hip_cal_done     : out std_logic                                         -- mcgb_hip_cal_done.hip_cal_done
	);
end entity lcpll_g3xn;

architecture rtl of lcpll_g3xn is
begin

	enable_pll_reconfig_check : if enable_pll_reconfig /= 1 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	rcfg_jtag_enable_check : if rcfg_jtag_enable /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	rcfg_separate_avmm_busy_check : if rcfg_separate_avmm_busy /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	dbg_embedded_debug_enable_check : if dbg_embedded_debug_enable /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	dbg_capability_reg_enable_check : if dbg_capability_reg_enable /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	dbg_user_identifier_check : if dbg_user_identifier /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	dbg_stat_soft_logic_enable_check : if dbg_stat_soft_logic_enable /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	dbg_ctrl_soft_logic_enable_check : if dbg_ctrl_soft_logic_enable /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	rcfg_emb_strm_enable_check : if rcfg_emb_strm_enable /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	rcfg_profile_cnt_check : if rcfg_profile_cnt /= 2 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_powerdown_mode_check : if hssi_pma_lc_refclk_select_mux_powerdown_mode /= "powerup" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_refclk_select_check : if hssi_pma_lc_refclk_select_mux_refclk_select /= "ref_iqclk0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_silicon_rev_check : if hssi_pma_lc_refclk_select_mux_silicon_rev /= "20nm5" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_inclk0_logical_to_physical_mapping_check : if hssi_pma_lc_refclk_select_mux_inclk0_logical_to_physical_mapping /= "ref_iqclk0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_inclk1_logical_to_physical_mapping_check : if hssi_pma_lc_refclk_select_mux_inclk1_logical_to_physical_mapping /= "power_down" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_inclk2_logical_to_physical_mapping_check : if hssi_pma_lc_refclk_select_mux_inclk2_logical_to_physical_mapping /= "power_down" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_inclk3_logical_to_physical_mapping_check : if hssi_pma_lc_refclk_select_mux_inclk3_logical_to_physical_mapping /= "power_down" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_lc_refclk_select_mux_inclk4_logical_to_physical_mapping_check : if hssi_pma_lc_refclk_select_mux_inclk4_logical_to_physical_mapping /= "power_down" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_refclk_divider_silicon_rev_check : if hssi_refclk_divider_silicon_rev /= "20nm5" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_silicon_rev_check : if atx_pll_silicon_rev /= "20nm5" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_is_cascaded_pll_check : if atx_pll_is_cascaded_pll /= "false" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_cgb_div_check : if atx_pll_cgb_div /= 1 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_pma_width_check : if atx_pll_pma_width /= 32 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_cp_compensation_enable_check : if atx_pll_cp_compensation_enable /= "true" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_cp_current_setting_check : if atx_pll_cp_current_setting /= "cp_current_setting26" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_cp_testmode_check : if atx_pll_cp_testmode /= "cp_normal" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_cp_lf_3rd_pole_freq_check : if atx_pll_cp_lf_3rd_pole_freq /= "lf_3rd_pole_setting0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_lf_cbig_size_check : if atx_pll_lf_cbig_size /= "lf_cbig_setting4" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_cp_lf_order_check : if atx_pll_cp_lf_order /= "lf_3rd_order" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_lf_resistance_check : if atx_pll_lf_resistance /= "lf_setting1" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_lf_ripplecap_check : if atx_pll_lf_ripplecap /= "lf_ripple_cap_0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_tank_sel_check : if atx_pll_tank_sel /= "lctank0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_tank_band_check : if atx_pll_tank_band /= "lc_band3" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_tank_voltage_coarse_check : if atx_pll_tank_voltage_coarse /= "vreg_setting_coarse0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_tank_voltage_fine_check : if atx_pll_tank_voltage_fine /= "vreg_setting5" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_output_regulator_supply_check : if atx_pll_output_regulator_supply /= "vreg1v_setting0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_overrange_voltage_check : if atx_pll_overrange_voltage /= "over_setting0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_underrange_voltage_check : if atx_pll_underrange_voltage /= "under_setting4" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_fb_select_check : if atx_pll_fb_select /= "direct_fb" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_d2a_voltage_check : if atx_pll_d2a_voltage /= "d2a_setting_4" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_dsm_mode_check : if atx_pll_dsm_mode /= "dsm_mode_integer" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_dsm_out_sel_check : if atx_pll_dsm_out_sel /= "pll_dsm_disable" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_dsm_ecn_bypass_check : if atx_pll_dsm_ecn_bypass /= "false" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_dsm_ecn_test_en_check : if atx_pll_dsm_ecn_test_en /= "false" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_dsm_fractional_division_check : if atx_pll_dsm_fractional_division /= "1" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_dsm_fractional_value_ready_check : if atx_pll_dsm_fractional_value_ready /= "pll_k_ready" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_iqclk_mux_sel_check : if atx_pll_iqclk_mux_sel /= "iqtxrxclk0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_vco_bypass_enable_check : if atx_pll_vco_bypass_enable /= "false" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_l_counter_check : if atx_pll_l_counter /= 2 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_l_counter_enable_check : if atx_pll_l_counter_enable /= "true" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_cascadeclk_test_check : if atx_pll_cascadeclk_test /= "cascadetest_off" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_hclk_divide_check : if atx_pll_hclk_divide /= 50 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_enable_hclk_check : if atx_pll_enable_hclk /= "hclk_disabled" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_m_counter_check : if atx_pll_m_counter /= 40 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_ref_clk_div_check : if atx_pll_ref_clk_div /= 1 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_bw_sel_check : if atx_pll_bw_sel /= "high" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_datarate_check : if atx_pll_datarate /= "8000000000 bps" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_device_variant_check : if atx_pll_device_variant /= "device1" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_initial_settings_check : if atx_pll_initial_settings /= "true" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_lc_mode_check : if atx_pll_lc_mode /= "lccmu_normal" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_output_clock_frequency_check : if atx_pll_output_clock_frequency /= "4000000000 Hz" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_powerdown_mode_check : if atx_pll_powerdown_mode /= "powerup" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_prot_mode_check : if atx_pll_prot_mode /= "pcie_gen3_tx" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_reference_clock_frequency_check : if atx_pll_reference_clock_frequency /= "100000000 Hz" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_sup_mode_check : if atx_pll_sup_mode /= "user_mode" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_regulator_bypass_check : if atx_pll_regulator_bypass /= "reg_enable" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_vco_freq_check : if atx_pll_vco_freq /= "8000000000 Hz" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_is_otn_check : if atx_pll_is_otn /= "false" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_is_sdi_check : if atx_pll_is_sdi /= "false" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_primary_use_check : if atx_pll_primary_use /= "hssi_x1" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_fpll_refclk_selection_check : if atx_pll_fpll_refclk_selection /= "select_vco_output" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_lc_to_fpll_l_counter_scratch_check : if atx_pll_lc_to_fpll_l_counter_scratch /= 1 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_lc_to_fpll_l_counter_check : if atx_pll_lc_to_fpll_l_counter /= "lcounter_setting0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_pfd_delay_compensation_check : if atx_pll_pfd_delay_compensation /= "normal_delay" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_xcpvco_xchgpmplf_cp_current_boost_check : if atx_pll_xcpvco_xchgpmplf_cp_current_boost /= "normal_setting" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_pfd_pulse_width_check : if atx_pll_pfd_pulse_width /= "pulse_width_setting0" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hip_cal_en_check : if hip_cal_en /= "enable" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	calibration_en_check : if calibration_en /= "enable" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	enable_analog_resets_check : if enable_analog_resets /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	atx_pll_bonding_mode_check : if atx_pll_bonding_mode /= "cpri_bonding" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	enable_mcgb_check : if enable_mcgb /= 1 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	enable_mcgb_debug_ports_parameters_check : if enable_mcgb_debug_ports_parameters /= 0 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_prot_mode_check : if hssi_pma_cgb_master_prot_mode /= "pcie_gen3_tx" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_silicon_rev_check : if hssi_pma_cgb_master_silicon_rev /= "20nm5" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_x1_div_m_sel_check : if hssi_pma_cgb_master_x1_div_m_sel /= "divbypass" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_cgb_enable_iqtxrxclk_check : if hssi_pma_cgb_master_cgb_enable_iqtxrxclk /= "disable_iqtxrxclk" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_ser_mode_check : if hssi_pma_cgb_master_ser_mode /= "thirty_two_bit" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_datarate_check : if hssi_pma_cgb_master_datarate /= "8000000000 bps" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_cgb_power_down_check : if hssi_pma_cgb_master_cgb_power_down /= "normal_cgb" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_observe_cgb_clocks_check : if hssi_pma_cgb_master_observe_cgb_clocks /= "observe_nothing" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_op_mode_check : if hssi_pma_cgb_master_op_mode /= "enabled" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_tx_ucontrol_reset_pcie_check : if hssi_pma_cgb_master_tx_ucontrol_reset_pcie /= "pcscorehip_controls_mcgb" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_vccdreg_output_check : if hssi_pma_cgb_master_vccdreg_output /= "vccdreg_nominal" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_input_select_check : if hssi_pma_cgb_master_input_select /= "fpll_top" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	hssi_pma_cgb_master_input_select_gen3_check : if hssi_pma_cgb_master_input_select_gen3 /= "lcpll_top" generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	lcpll_g3xn : component ip_pcie_x4_256_altera_xcvr_atx_pll_a10_181.lcpll_g3xn_pkg.ip_pcie_x4_256_altera_xcvr_atx_pll_a10_181_6kz3oaa
		generic map (
			enable_pll_reconfig                                              => 1,
			rcfg_jtag_enable                                                 => 0,
			rcfg_separate_avmm_busy                                          => 0,
			dbg_embedded_debug_enable                                        => 0,
			dbg_capability_reg_enable                                        => 0,
			dbg_user_identifier                                              => 0,
			dbg_stat_soft_logic_enable                                       => 0,
			dbg_ctrl_soft_logic_enable                                       => 0,
			rcfg_emb_strm_enable                                             => 0,
			rcfg_profile_cnt                                                 => 2,
			hssi_pma_lc_refclk_select_mux_powerdown_mode                     => "powerup",
			hssi_pma_lc_refclk_select_mux_refclk_select                      => "ref_iqclk0",
			hssi_pma_lc_refclk_select_mux_silicon_rev                        => "20nm5",
			hssi_pma_lc_refclk_select_mux_inclk0_logical_to_physical_mapping => "ref_iqclk0",
			hssi_pma_lc_refclk_select_mux_inclk1_logical_to_physical_mapping => "power_down",
			hssi_pma_lc_refclk_select_mux_inclk2_logical_to_physical_mapping => "power_down",
			hssi_pma_lc_refclk_select_mux_inclk3_logical_to_physical_mapping => "power_down",
			hssi_pma_lc_refclk_select_mux_inclk4_logical_to_physical_mapping => "power_down",
			hssi_refclk_divider_silicon_rev                                  => "20nm5",
			atx_pll_silicon_rev                                              => "20nm5",
			atx_pll_is_cascaded_pll                                          => "false",
			atx_pll_cgb_div                                                  => 1,
			atx_pll_pma_width                                                => 32,
			atx_pll_cp_compensation_enable                                   => "true",
			atx_pll_cp_current_setting                                       => "cp_current_setting26",
			atx_pll_cp_testmode                                              => "cp_normal",
			atx_pll_cp_lf_3rd_pole_freq                                      => "lf_3rd_pole_setting0",
			atx_pll_lf_cbig_size                                             => "lf_cbig_setting4",
			atx_pll_cp_lf_order                                              => "lf_3rd_order",
			atx_pll_lf_resistance                                            => "lf_setting1",
			atx_pll_lf_ripplecap                                             => "lf_ripple_cap_0",
			atx_pll_tank_sel                                                 => "lctank0",
			atx_pll_tank_band                                                => "lc_band3",
			atx_pll_tank_voltage_coarse                                      => "vreg_setting_coarse0",
			atx_pll_tank_voltage_fine                                        => "vreg_setting5",
			atx_pll_output_regulator_supply                                  => "vreg1v_setting0",
			atx_pll_overrange_voltage                                        => "over_setting0",
			atx_pll_underrange_voltage                                       => "under_setting4",
			atx_pll_fb_select                                                => "direct_fb",
			atx_pll_d2a_voltage                                              => "d2a_setting_4",
			atx_pll_dsm_mode                                                 => "dsm_mode_integer",
			atx_pll_dsm_out_sel                                              => "pll_dsm_disable",
			atx_pll_dsm_ecn_bypass                                           => "false",
			atx_pll_dsm_ecn_test_en                                          => "false",
			atx_pll_dsm_fractional_division                                  => "1",
			atx_pll_dsm_fractional_value_ready                               => "pll_k_ready",
			atx_pll_iqclk_mux_sel                                            => "iqtxrxclk0",
			atx_pll_vco_bypass_enable                                        => "false",
			atx_pll_l_counter                                                => 2,
			atx_pll_l_counter_enable                                         => "true",
			atx_pll_cascadeclk_test                                          => "cascadetest_off",
			atx_pll_hclk_divide                                              => 50,
			atx_pll_enable_hclk                                              => "hclk_disabled",
			atx_pll_m_counter                                                => 40,
			atx_pll_ref_clk_div                                              => 1,
			atx_pll_bw_sel                                                   => "high",
			atx_pll_datarate                                                 => "8000000000 bps",
			atx_pll_device_variant                                           => "device1",
			atx_pll_initial_settings                                         => "true",
			atx_pll_lc_mode                                                  => "lccmu_normal",
			atx_pll_output_clock_frequency                                   => "4000000000 Hz",
			atx_pll_powerdown_mode                                           => "powerup",
			atx_pll_prot_mode                                                => "pcie_gen3_tx",
			atx_pll_reference_clock_frequency                                => "100000000 Hz",
			atx_pll_sup_mode                                                 => "user_mode",
			atx_pll_regulator_bypass                                         => "reg_enable",
			atx_pll_vco_freq                                                 => "8000000000 Hz",
			atx_pll_is_otn                                                   => "false",
			atx_pll_is_sdi                                                   => "false",
			atx_pll_primary_use                                              => "hssi_x1",
			atx_pll_fpll_refclk_selection                                    => "select_vco_output",
			atx_pll_lc_to_fpll_l_counter_scratch                             => 1,
			atx_pll_lc_to_fpll_l_counter                                     => "lcounter_setting0",
			atx_pll_pfd_delay_compensation                                   => "normal_delay",
			atx_pll_xcpvco_xchgpmplf_cp_current_boost                        => "normal_setting",
			atx_pll_pfd_pulse_width                                          => "pulse_width_setting0",
			hip_cal_en                                                       => "enable",
			calibration_en                                                   => "enable",
			enable_analog_resets                                             => 0,
			atx_pll_bonding_mode                                             => "cpri_bonding",
			enable_mcgb                                                      => 1,
			enable_mcgb_debug_ports_parameters                               => 0,
			hssi_pma_cgb_master_prot_mode                                    => "pcie_gen3_tx",
			hssi_pma_cgb_master_silicon_rev                                  => "20nm5",
			hssi_pma_cgb_master_x1_div_m_sel                                 => "divbypass",
			hssi_pma_cgb_master_cgb_enable_iqtxrxclk                         => "disable_iqtxrxclk",
			hssi_pma_cgb_master_ser_mode                                     => "thirty_two_bit",
			hssi_pma_cgb_master_datarate                                     => "8000000000 bps",
			hssi_pma_cgb_master_cgb_power_down                               => "normal_cgb",
			hssi_pma_cgb_master_observe_cgb_clocks                           => "observe_nothing",
			hssi_pma_cgb_master_op_mode                                      => "enabled",
			hssi_pma_cgb_master_tx_ucontrol_reset_pcie                       => "pcscorehip_controls_mcgb",
			hssi_pma_cgb_master_vccdreg_output                               => "vccdreg_nominal",
			hssi_pma_cgb_master_input_select                                 => "fpll_top",
			hssi_pma_cgb_master_input_select_gen3                            => "lcpll_top"
		)
		port map (
			pll_powerdown           => pll_powerdown,                      --     pll_powerdown.pll_powerdown
			pll_refclk0             => pll_refclk0,                        --       pll_refclk0.clk
			pll_locked              => pll_locked,                         --        pll_locked.pll_locked
			pll_pcie_clk            => pll_pcie_clk,                       --      pll_pcie_clk.pll_pcie_clk
			reconfig_clk0           => reconfig_clk0,                      --     reconfig_clk0.clk
			reconfig_reset0         => reconfig_reset0,                    --   reconfig_reset0.reset
			reconfig_write0         => reconfig_write0,                    --    reconfig_avmm0.write
			reconfig_read0          => reconfig_read0,                     --                  .read
			reconfig_address0       => reconfig_address0,                  --                  .address
			reconfig_writedata0     => reconfig_writedata0,                --                  .writedata
			reconfig_readdata0      => reconfig_readdata0,                 --                  .readdata
			reconfig_waitrequest0   => reconfig_waitrequest0,              --                  .waitrequest
			pll_cal_busy            => pll_cal_busy,                       --      pll_cal_busy.pll_cal_busy
			hip_cal_done            => hip_cal_done,                       --      hip_cal_done.hip_cal_done
			mcgb_rst                => mcgb_rst,                           --          mcgb_rst.mcgb_rst
			mcgb_aux_clk0           => mcgb_aux_clk0,                      --     mcgb_aux_clk0.tx_serial_clk
			tx_bonding_clocks       => tx_bonding_clocks,                  -- tx_bonding_clocks.clk
			pcie_sw                 => pcie_sw,                            --           pcie_sw.pcie_sw
			pcie_sw_done            => pcie_sw_done,                       --      pcie_sw_done.pcie_sw_done
			mcgb_hip_cal_done       => mcgb_hip_cal_done,                  -- mcgb_hip_cal_done.hip_cal_done
			pll_refclk1             => '0',                                --       (terminated)
			pll_refclk2             => '0',                                --       (terminated)
			pll_refclk3             => '0',                                --       (terminated)
			pll_refclk4             => '0',                                --       (terminated)
			tx_serial_clk           => open,                               --       (terminated)
			tx_serial_clk_gt        => open,                               --       (terminated)
			pll_cascade_clk         => open,                               --       (terminated)
			atx_to_fpll_cascade_clk => open,                               --       (terminated)
			avmm_busy0              => open,                               --       (terminated)
			clklow                  => open,                               --       (terminated)
			fref                    => open,                               --       (terminated)
			overrange               => open,                               --       (terminated)
			underrange              => open,                               --       (terminated)
			mcgb_aux_clk1           => '0',                                --       (terminated)
			mcgb_aux_clk2           => '0',                                --       (terminated)
			mcgb_serial_clk         => open,                               --       (terminated)
			reconfig_clk1           => '0',                                --       (terminated)
			reconfig_reset1         => '0',                                --       (terminated)
			reconfig_write1         => '0',                                --       (terminated)
			reconfig_read1          => '0',                                --       (terminated)
			reconfig_address1       => "0000000000",                       --       (terminated)
			reconfig_writedata1     => "00000000000000000000000000000000", --       (terminated)
			reconfig_readdata1      => open,                               --       (terminated)
			reconfig_waitrequest1   => open,                               --       (terminated)
			mcgb_cal_busy           => open                                --       (terminated)
		);

end architecture rtl; -- of lcpll_g3xn
