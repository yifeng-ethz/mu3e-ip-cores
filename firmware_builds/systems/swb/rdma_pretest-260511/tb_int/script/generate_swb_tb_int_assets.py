...
IMPLEMENTED_CASES: list[Case] = [
    ...
    Case("B065", "BASIC", "D", "DT", "SWB DT one SQE ingress through OPQ to PCIe DMA egress", "inject one FEB-to-SWB RDMA SQE, one OPQ packet, and one host-DMA beat", "scoreboard reconciles one ingress, zero drops, and one PCIe egress event", "uvm/swb_rdma_pretest/tests/tb_int_b065_test.sv"),
    Case("B066", "BASIC", "D", "DT", "SWB DT rdma_subsystem CQE round-trip", "drive one legal SQE and observe one CQE completion", "SQ consumed count and CQ posted count both advance by one", "uvm/swb_rdma_pretest/tests/tb_int_b066_test.sv"),
    Case("B067", "BASIC", "D", "DT", "SWB DT sidecar lineage at FEB-side rdma SQE ingress", "drive one SQE with DEBUG_LEVEL 2 sidecar identity", "nominal payload and sidecar identity reconcile through the selected monitors", "uvm/swb_rdma_pretest/tests/tb_int_b067_test.sv"),
    ...
    Case("X001", "ERROR", "D", "RC", "mid-flight RESET while OPQ has SQEs in flight", "accept one SQE/OPQ packet then force the reset-drain model", "in-flight packet is accounted as dropped and no DMA event is emitted", "uvm/swb_rdma_pretest/tests/tb_int_x001_test.sv"),
    ...
    Case("X065", "ERROR", "D", "DT", "rdma CQE timeout", "drive one SQE then suppress CQE writeback", "SQE ingress is observed and CQE count remains zero", "uvm/swb_rdma_pretest/tests/tb_int_x065_test.sv"),
    ...
    Case("P068", "PROF", "D", "DT", "sustained SQE ingress at line-rate structural scale", "drive a 128-SQE structural line-rate burst", "SQE, OPQ, and DMA ledgers reconcile without halt", "uvm/swb_rdma_pretest/tests/tb_int_p068_test.sv"),
]
...
def fallback_row(prefix: str, bucket: str, section: str, number: int) -> str:
    ...
    return line_row(
        ...
        f"{bucket} DT reserved SQE/OPQ/DMA sweep seed {seed}",
        1,
...
def emit_bucket(bucket: str) -> str:
    ...
    lines: list[str] = [
        ...
        f"| DT | 128 | {prefix}065-{prefix}192 | FEB-to-SWB RDMA SQE traffic is reconciled by the per-stage ledger scoreboard | {counts['DT']}/128 |",
    ]
...
def emit_dv_report() -> str:
    ...
    lines.extend([
        ...
        f"| PASS | [`swb_selected_22_case_sweep`](REPORT/README.md) | isolated_sweep | rdma_pretest-260511 | selected BASIC/EDGE/ERROR/PROF cases | {implemented} | n/a |",
        "",
        "## Index",
        "",
...
def emit_dv_cov() -> str:
    lines = [
        ...
        "## 1. Scope",
        "",
        "This coverage skeleton tracks the selected 22-case structural UVM smoke. UCDB code coverage is intentionally `n/a` until the real mixed-language DUT bind is enabled.",
        "",
