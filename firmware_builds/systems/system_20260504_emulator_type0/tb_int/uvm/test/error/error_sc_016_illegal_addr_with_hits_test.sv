// error_sc_016_illegal_addr_with_hits_test.sv
// ERROR bucket directed test for ERROR-SC-016.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_sc_016_illegal_addr_with_hits_test extends tb_int_base_test;
    `uvm_component_utils(error_sc_016_illegal_addr_with_hits_test)
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        seq_obj = uvm_factory::get().create_object_by_name("error_sc_016_illegal_addr_with_hits_seq", "", "error_sc_016_illegal_addr_with_hits_seq");
        if (!$cast(seq, seq_obj))
            `uvm_fatal("ERROR_TEST", "factory could not create error_sc_016_illegal_addr_with_hits_seq")
        seq.start(null);
        $display("*** TEST PASSED ***");
...
module error_sc_016_illegal_addr_with_hits_test_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_sc_016_illegal_addr_with_hits_test::type_id::get();
endmodule

bind tb_int_top error_sc_016_illegal_addr_with_hits_test_factory_anchor u_error_sc_016_illegal_addr_with_hits_test_factory_anchor();
