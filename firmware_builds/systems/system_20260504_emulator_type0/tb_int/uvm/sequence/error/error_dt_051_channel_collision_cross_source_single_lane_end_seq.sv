// error_dt_051_channel_collision_cross_source_single_lane_end_seq.sv
// ERROR bucket directed sequence for ERROR-DT-051.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_051_channel_collision_cross_source_single_lane_end_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_051_channel_collision_cross_source_single_lane_end_seq)

    function new(string name = "error_dt_051_channel_collision_cross_source_single_lane_end_seq");
        super.new(name);
    endfunction
    ...
    task automatic drive_hit_path(
        ...
        for (int unsigned hit_idx = 0; hit_idx < hit_count; hit_idx++) begin
            lane_id = 4'((51 + hit_idx) % 2);
            channel = 5'((9 + hit_idx) % 16);
            if (pattern == 0 && 1 != 0)
                channel = 5'd8;
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-051: Real and emulator sources collide on channel 8 in MIX_RR; variant single lane end", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-051 expected_syndrome=0x%08h pattern=0 hits=8", 32'h00006403),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_051_channel_collision_cross_source_single_lane_end_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_051_channel_collision_cross_source_single_lane_end_seq::type_id::get();
endmodule

bind tb_int_top error_dt_051_channel_collision_cross_source_single_lane_end_seq_factory_anchor u_error_dt_051_channel_collision_cross_source_single_lane_end_seq_factory_anchor();
