// error_dt_095_double_eop_without_sop_reset_overlap_seq.sv
// ERROR bucket directed sequence for ERROR-DT-095.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_095_double_eop_without_sop_reset_overlap_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_095_double_eop_without_sop_reset_overlap_seq)

    function new(string name = "error_dt_095_double_eop_without_sop_reset_overlap_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-095: Extra EOP-marker beat appears without a matching SOP; variant reset overlap", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-095 expected_syndrome=0x%08h pattern=4 hits=8", 32'h0000660f),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_095_double_eop_without_sop_reset_overlap_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_095_double_eop_without_sop_reset_overlap_seq::type_id::get();
endmodule

bind tb_int_top error_dt_095_double_eop_without_sop_reset_overlap_seq_factory_anchor u_error_dt_095_double_eop_without_sop_reset_overlap_seq_factory_anchor();
