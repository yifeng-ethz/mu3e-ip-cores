// error_rc_009_b2b_reset_run_prep_seq.sv
// ERROR bucket directed sequence for ERROR-RC-009.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_rc_009_b2b_reset_run_prep_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_rc_009_b2b_reset_run_prep_seq)

    function new(string name = "error_rc_009_b2b_reset_run_prep_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-RC-009: Back-to-back RESETs while returning from RUN_PREP", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-RC-009 expected_syndrome=0x%08h pattern=0 hits=4", 32'h00002009),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_rc_009_b2b_reset_run_prep_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_rc_009_b2b_reset_run_prep_seq::type_id::get();
endmodule

bind tb_int_top error_rc_009_b2b_reset_run_prep_seq_factory_anchor u_error_rc_009_b2b_reset_run_prep_seq_factory_anchor();
