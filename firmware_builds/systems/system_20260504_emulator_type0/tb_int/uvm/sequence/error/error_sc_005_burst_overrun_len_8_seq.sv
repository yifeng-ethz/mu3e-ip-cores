// error_sc_005_burst_overrun_len_8_seq.sv
// ERROR bucket directed sequence for ERROR-SC-005.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_sc_005_burst_overrun_len_8_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_sc_005_burst_overrun_len_8_seq)

    function new(string name = "error_sc_005_burst_overrun_len_8_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-SC-005: Eight-beat burst crosses the 4 KB aperture", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-SC-005 expected_syndrome=0x%08h pattern=0 hits=5", 32'h00005105),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_sc_005_burst_overrun_len_8_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_sc_005_burst_overrun_len_8_seq::type_id::get();
endmodule

bind tb_int_top error_sc_005_burst_overrun_len_8_seq_factory_anchor u_error_sc_005_burst_overrun_len_8_seq_factory_anchor();
