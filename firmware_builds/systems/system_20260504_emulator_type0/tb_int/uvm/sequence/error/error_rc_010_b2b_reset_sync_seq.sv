// error_rc_010_b2b_reset_sync_seq.sv
// ERROR bucket directed sequence for ERROR-RC-010.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_rc_010_b2b_reset_sync_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_rc_010_b2b_reset_sync_seq)

    function new(string name = "error_rc_010_b2b_reset_sync_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-RC-010: Back-to-back RESETs while returning from SYNC", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-RC-010 expected_syndrome=0x%08h pattern=0 hits=5", 32'h0000200a),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_rc_010_b2b_reset_sync_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_rc_010_b2b_reset_sync_seq::type_id::get();
endmodule

bind tb_int_top error_rc_010_b2b_reset_sync_seq_factory_anchor u_error_rc_010_b2b_reset_sync_seq_factory_anchor();
