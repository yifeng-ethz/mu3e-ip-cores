// error_dt_014_bad_crc_frame_end_phase_seq.sv
// ERROR bucket directed sequence for ERROR-DT-014.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_014_bad_crc_frame_end_phase_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_014_bad_crc_frame_end_phase_seq)

    function new(string name = "error_dt_014_bad_crc_frame_end_phase_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-014: Bad CRC injected in the virtual MuTRiG frame trailer; variant frame end phase", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-014 expected_syndrome=0x%08h pattern=1 hits=7", 32'h0000610e),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_014_bad_crc_frame_end_phase_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_014_bad_crc_frame_end_phase_seq::type_id::get();
endmodule

bind tb_int_top error_dt_014_bad_crc_frame_end_phase_seq_factory_anchor u_error_dt_014_bad_crc_frame_end_phase_seq_factory_anchor();
