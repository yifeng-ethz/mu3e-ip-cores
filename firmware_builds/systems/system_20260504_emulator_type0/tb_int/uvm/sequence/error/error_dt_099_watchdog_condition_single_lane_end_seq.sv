// error_dt_099_watchdog_condition_single_lane_end_seq.sv
// ERROR bucket directed sequence for ERROR-DT-099.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_099_watchdog_condition_single_lane_end_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_099_watchdog_condition_single_lane_end_seq)

    function new(string name = "error_dt_099_watchdog_condition_single_lane_end_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-099: Watchdog firing and non-firing windows under source silence; variant single lane end", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-099 expected_syndrome=0x%08h pattern=1 hits=6", 32'h00006703),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_099_watchdog_condition_single_lane_end_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_099_watchdog_condition_single_lane_end_seq::type_id::get();
endmodule

bind tb_int_top error_dt_099_watchdog_condition_single_lane_end_seq_factory_anchor u_error_dt_099_watchdog_condition_single_lane_end_seq_factory_anchor();
