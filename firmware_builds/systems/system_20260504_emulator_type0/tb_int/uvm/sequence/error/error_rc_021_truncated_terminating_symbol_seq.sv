// error_rc_021_truncated_terminating_symbol_seq.sv
// ERROR bucket directed sequence for ERROR-RC-021.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_rc_021_truncated_terminating_symbol_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_rc_021_truncated_terminating_symbol_seq)

    function new(string name = "error_rc_021_truncated_terminating_symbol_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-RC-021: Truncated RC payload while reporting TERMINATING", UVM_LOW)
        tb_int_run_window_db::reset();
        drive_run_symbol(runctl_vif, 9'h002, 2);
        if (3 == 3)
            drive_truncated_run_symbol(runctl_vif, 9'(16));
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-RC-021 expected_syndrome=0x%08h pattern=0 hits=4", 32'h00003015),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_rc_021_truncated_terminating_symbol_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_rc_021_truncated_terminating_symbol_seq::type_id::get();
endmodule

bind tb_int_top error_rc_021_truncated_terminating_symbol_seq_factory_anchor u_error_rc_021_truncated_terminating_symbol_seq_factory_anchor();
