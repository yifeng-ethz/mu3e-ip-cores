// error_dt_065_sop_without_eop_single_lane_start_seq.sv
// ERROR bucket directed sequence for ERROR-DT-065.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_065_sop_without_eop_single_lane_start_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_065_sop_without_eop_single_lane_start_seq)

    function new(string name = "error_dt_065_sop_without_eop_single_lane_start_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-065: SOP observed without a matching EOP from one source; variant single lane start", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-065 expected_syndrome=0x%08h pattern=2 hits=4", 32'h00006501),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_065_sop_without_eop_single_lane_start_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_065_sop_without_eop_single_lane_start_seq::type_id::get();
endmodule

bind tb_int_top error_dt_065_sop_without_eop_single_lane_start_seq_factory_anchor u_error_dt_065_sop_without_eop_single_lane_start_seq_factory_anchor();
