// error_rc_003_mid_flush_hist_bin_commit_seq.sv
// ERROR bucket directed sequence for ERROR-RC-003.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_rc_003_mid_flush_hist_bin_commit_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_rc_003_mid_flush_hist_bin_commit_seq)

    function new(string name = "error_rc_003_mid_flush_hist_bin_commit_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-RC-003: RESET during histogram_statistics mid-bin commit", UVM_LOW)
        tb_int_run_window_db::reset();
        drive_run_symbol(runctl_vif, 9'h002, 2);
        if (1 == 3)
            drive_truncated_run_symbol(runctl_vif, 9'(4));
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-RC-003 expected_syndrome=0x%08h pattern=0 hits=4", 32'h00001003),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_rc_003_mid_flush_hist_bin_commit_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_rc_003_mid_flush_hist_bin_commit_seq::type_id::get();
endmodule

bind tb_int_top error_rc_003_mid_flush_hist_bin_commit_seq_factory_anchor u_error_rc_003_mid_flush_hist_bin_commit_seq_factory_anchor();
