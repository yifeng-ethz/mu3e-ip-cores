// error_dt_079_sop_without_eop_reset_overlap_seq.sv
// ERROR bucket directed sequence for ERROR-DT-079.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_079_sop_without_eop_reset_overlap_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_079_sop_without_eop_reset_overlap_seq)

    function new(string name = "error_dt_079_sop_without_eop_reset_overlap_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-079: SOP observed without a matching EOP from one source; variant reset overlap", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-079 expected_syndrome=0x%08h pattern=2 hits=8", 32'h0000650f),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_079_sop_without_eop_reset_overlap_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_079_sop_without_eop_reset_overlap_seq::type_id::get();
endmodule

bind tb_int_top error_dt_079_sop_without_eop_reset_overlap_seq_factory_anchor u_error_dt_079_sop_without_eop_reset_overlap_seq_factory_anchor();
