// error_rc_022_truncated_reset_symbol_seq.sv
// ERROR bucket directed sequence for ERROR-RC-022.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_rc_022_truncated_reset_symbol_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_rc_022_truncated_reset_symbol_seq)

    function new(string name = "error_rc_022_truncated_reset_symbol_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-RC-022: Truncated RC payload while reporting RESET", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-RC-022 expected_syndrome=0x%08h pattern=0 hits=4", 32'h00003016),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_rc_022_truncated_reset_symbol_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_rc_022_truncated_reset_symbol_seq::type_id::get();
endmodule

bind tb_int_top error_rc_022_truncated_reset_symbol_seq_factory_anchor u_error_rc_022_truncated_reset_symbol_seq_factory_anchor();
