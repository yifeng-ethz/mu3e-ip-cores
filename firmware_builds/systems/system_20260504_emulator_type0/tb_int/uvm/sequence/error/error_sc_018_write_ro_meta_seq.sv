// error_sc_018_write_ro_meta_seq.sv
// ERROR bucket directed sequence for ERROR-SC-018.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_sc_018_write_ro_meta_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_sc_018_write_ro_meta_seq)

    function new(string name = "error_sc_018_write_ro_meta_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-SC-018: Write attempt to META read-only field", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-SC-018 expected_syndrome=0x%08h pattern=0 hits=4", 32'h00005312),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_sc_018_write_ro_meta_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_sc_018_write_ro_meta_seq::type_id::get();
endmodule

bind tb_int_top error_sc_018_write_ro_meta_seq_factory_anchor u_error_sc_018_write_ro_meta_seq_factory_anchor();
