// error_dt_109_watchdog_condition_frame_start_phase_seq.sv
// ERROR bucket directed sequence for ERROR-DT-109.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_109_watchdog_condition_frame_start_phase_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_109_watchdog_condition_frame_start_phase_seq)

    function new(string name = "error_dt_109_watchdog_condition_frame_start_phase_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-109: Watchdog firing and non-firing windows under source silence; variant frame start phase", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-109 expected_syndrome=0x%08h pattern=0 hits=6", 32'h0000670d),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_109_watchdog_condition_frame_start_phase_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_109_watchdog_condition_frame_start_phase_seq::type_id::get();
endmodule

bind tb_int_top error_dt_109_watchdog_condition_frame_start_phase_seq_factory_anchor u_error_dt_109_watchdog_condition_frame_start_phase_seq_factory_anchor();
