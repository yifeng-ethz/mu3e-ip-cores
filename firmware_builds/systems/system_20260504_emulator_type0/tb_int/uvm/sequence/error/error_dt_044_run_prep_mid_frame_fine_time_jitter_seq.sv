// error_dt_044_run_prep_mid_frame_fine_time_jitter_seq.sv
// ERROR bucket directed sequence for ERROR-DT-044.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_044_run_prep_mid_frame_fine_time_jitter_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_044_run_prep_mid_frame_fine_time_jitter_seq)

    function new(string name = "error_dt_044_run_prep_mid_frame_fine_time_jitter_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-044: RUN_PREP injected while a frame is open; variant fine time jitter", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-044 expected_syndrome=0x%08h pattern=2 hits=5", 32'h0000630c),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_044_run_prep_mid_frame_fine_time_jitter_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_044_run_prep_mid_frame_fine_time_jitter_seq::type_id::get();
endmodule

bind tb_int_top error_dt_044_run_prep_mid_frame_fine_time_jitter_seq_factory_anchor u_error_dt_044_run_prep_mid_frame_fine_time_jitter_seq_factory_anchor();
