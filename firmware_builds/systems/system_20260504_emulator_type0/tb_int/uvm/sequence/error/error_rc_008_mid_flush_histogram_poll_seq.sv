// error_rc_008_mid_flush_histogram_poll_seq.sv
// ERROR bucket directed sequence for ERROR-RC-008.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_rc_008_mid_flush_histogram_poll_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_rc_008_mid_flush_histogram_poll_seq)

    function new(string name = "error_rc_008_mid_flush_histogram_poll_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-RC-008: RESET concurrent with histogram CSR poll", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-RC-008 expected_syndrome=0x%08h pattern=0 hits=6", 32'h00001008),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_rc_008_mid_flush_histogram_poll_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_rc_008_mid_flush_histogram_poll_seq::type_id::get();
endmodule

bind tb_int_top error_rc_008_mid_flush_histogram_poll_seq_factory_anchor u_error_rc_008_mid_flush_histogram_poll_seq_factory_anchor();
