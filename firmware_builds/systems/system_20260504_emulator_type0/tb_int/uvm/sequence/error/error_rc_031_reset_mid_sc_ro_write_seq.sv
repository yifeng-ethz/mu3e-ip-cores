// error_rc_031_reset_mid_sc_ro_write_seq.sv
// ERROR bucket directed sequence for ERROR-RC-031.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_rc_031_reset_mid_sc_ro_write_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_rc_031_reset_mid_sc_ro_write_seq)

    function new(string name = "error_rc_031_reset_mid_sc_ro_write_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-RC-031: RESET issued in the middle of a write-to-RO probe", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-RC-031 expected_syndrome=0x%08h pattern=0 hits=5", 32'h0000401f),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_rc_031_reset_mid_sc_ro_write_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_rc_031_reset_mid_sc_ro_write_seq::type_id::get();
endmodule

bind tb_int_top error_rc_031_reset_mid_sc_ro_write_seq_factory_anchor u_error_rc_031_reset_mid_sc_ro_write_seq_factory_anchor();
