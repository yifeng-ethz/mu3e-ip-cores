// error_dt_080_sop_without_eop_histogram_overlap_seq.sv
// ERROR bucket directed sequence for ERROR-DT-080.
// Author: Yifeng Wang
// Version : 26.2.0
// Date    : 20260505
...
class error_dt_080_sop_without_eop_histogram_overlap_seq extends uvm_sequence #(uvm_sequence_item);
    `uvm_object_utils(error_dt_080_sop_without_eop_histogram_overlap_seq)

    function new(string name = "error_dt_080_sop_without_eop_histogram_overlap_seq");
        super.new(name);
    endfunction
    ...
    virtual task body();
        ...
        `uvm_info("ERROR_CASE", "ERROR-DT-080: SOP observed without a matching EOP from one source; variant histogram overlap", UVM_LOW)
        ...
        `uvm_info("ERROR_CASE",
                  $sformatf("ERROR-DT-080 expected_syndrome=0x%08h pattern=2 hits=4", 32'h00006510),
                  UVM_LOW)
        repeat (8) @(posedge stage_a_vif.clk);
    endtask
endclass

module error_dt_080_sop_without_eop_histogram_overlap_seq_factory_anchor;
    uvm_object_wrapper anchor_wrapper = error_dt_080_sop_without_eop_histogram_overlap_seq::type_id::get();
endmodule

bind tb_int_top error_dt_080_sop_without_eop_histogram_overlap_seq_factory_anchor u_error_dt_080_sop_without_eop_histogram_overlap_seq_factory_anchor();
