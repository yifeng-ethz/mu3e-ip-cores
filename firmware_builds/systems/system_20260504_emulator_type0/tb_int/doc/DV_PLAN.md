...
The current PROF-INT-002 generated-system tests drive traffic from the generated internal `emulator_mutrig_qsys_lane` instances in the full8lane type0 Qsys system. These runs use the emulator's direct hit_type0 stream (`BYTE_STREAM_ENABLE=0`) and do not exercise a real external MuTRiG, the LVDS PHY pins, or `mutrig_frame_deassembly`; a future real-LVDS harness must bind a virtual MuTRiG at that upstream serial boundary. The integration datapath under test is therefore `emulator_mutrig_qsys_lane` -> `arb_hit_type0` -> `backpressure_fifo` -> `mutrig_timestamp_processor` -> `ring_buffer_cam` -> `histogram_ingress_bridge` -> `histogram_statistics`, reconciled end-to-end through a per-bucket FIFO-ledger scoreboard.
