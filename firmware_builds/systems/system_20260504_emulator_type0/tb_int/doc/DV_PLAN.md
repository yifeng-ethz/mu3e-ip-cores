...
- Reporter output requirement:
  ...
  - Post-rbCAM and FEB-egress panels must use the corrected expected latency apertures above. Under the 100 kHz/channel typical workload the FIFOs should be empty or near-empty, so post-rbCAM should be near the fixed rbCAM latency. FEB-egress may spread across the current-frame remainder and the following store-and-forward interval when the periodic source walks through frame phase; the closure gate is zero POST-to-FEB residual plus all DEBUG-age rows inside the FEB-egress aperture.
