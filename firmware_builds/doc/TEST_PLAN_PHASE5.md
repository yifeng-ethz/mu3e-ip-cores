...
2. **Stage cfg bitstream** for ASIC `k ∈ {0..7}`:
    - Compute the cfg-bitstream **word stream** from the repo-owned board-test helper `script/configure_mutrig_from_xml.py`. The online production packer at `/home/yifeng/packages/online_dpv2/online/switching_pc/slowcontrol/mutrig/Mutrig_FEB.cpp` and the FE SciFi toolkit are reference inputs only. The expected stream is 84 32-bit words holding the 2662-bit packed bitstream, padded to a word boundary; if word order is questioned, compare against online code as a clue, then prove the final choice with scratchpad readback and MuTRiG frame/hit evidence.
...
4. **Verify decoded frames** at `mutrig_frame_deassembly_k.csr`:
    ...
    - If any deassembler does not advance, halt. That ASIC was not configured, most likely because the scratchpad word stream was wrong. Use online code only as a comparison target; the fix must land in the repo-owned cfg helper or the hardware cfg path.
...
```text
  ...
  mts_processor_{0,1}        (mts_processor.vhd; gts_8n counter, mts→gts mapping)
     ...
     ▼
  histogram_ingress_bridge_0 / histogram_ingress_bridge_1 (pre/post tap selectors)
     │  bridge_0 forwards upper pre_in to hit_stack_subsystem_0
     │  bridge_1 taps lower pre-RBCAM hit_type1 through hist_pre_lower_splitter_0
     │  bridge_1.pre_out is drained so the lower histogram tap cannot backpressure MTS
     ▼
...
Current debug finding: the post-hit-stack rate path is collectively merged, so lower-side lane4 rate tests pass. The earlier delay path was upper-only because `mts_preprocessor_0.ts_delta` was wired to `histogram_statistics_0.debug_1` while `mts_preprocessor_1.ts_delta` was not connected. That source choice is now explicitly rejected for rbCAM latency: `ts_delta` is an inter-hit timestamp-delta diagnostic and produced the false exact-zero all-ASIC plots in `phase6_header_delay_asic_20260501_hdrch_mts1pct`. Source Qsys now uses `histogram_statistics_v2` mode `-7` with `debug_1=upper MTS debug_ts`, `debug_2=lower MTS debug_ts`, and `debug_3..6=upper ring-CAM fill levels`. The 26.1.5 RTL extends the debug path so CSR filtering can select debug source 0/1 inside mode `-7`; it does not by itself make a single MTS stream carry individual lane identity. The 2026-05-01 pipe regeneration fixes a separate rate-observability bug: `histogram_ingress_bridge_0` now defaults to the pre-RBCAM MTS stream, `histogram_statistics_0` has two fill inputs with `CHANNELS_PER_PORT=0` global `{ASIC,channel}` keys, and `hist_pre_lower_splitter_0` feeds both `hit_stack_subsystem_1` and `histogram_ingress_bridge_1`; `histogram_ingress_bridge_1.pre_out` is drained so the tap cannot stall MTS. Authentic generated-system simulation passed after this change with all eight lanes visible, 20 MTS channel-16 payload hits and 20 histogram flushes per lane, and zero histogram drops. The v26.1.6 no-STP image checksum `0x13A9CC2D` adds the last-interval rate counter and closes quick-CSR scoped real-rate evidence on lanes 0 and 3. Full all-lane real/mixed source evidence and final raw-bin DISLIN/System-Console histogram plot plus matching live closure are still open; the collapsed 2026-05-01 bin-0 rate plot is an anomaly, not a pass.
...
- **Cfg bitstream packing**: the repo-owned cfg helper owns Phase-5 board-test packing. `/home/yifeng/packages/online_dpv2/online/switching_pc/slowcontrol/mutrig/Mutrig_FEB.cpp` and `toolkits/fe_scifi/` are reference inputs for LSB/MSB and padding questions, not trusted signoff tools. Phase-5 §2 signs off the emitted 84-word stream through scratchpad readback and real MuTRiG frame/hit evidence.
